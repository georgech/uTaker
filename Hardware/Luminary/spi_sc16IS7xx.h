...
static unsigned char fnReadExtSCI_byte(QUEUE_HANDLE Channel, unsigned char ucAddress)
{
    ...
    ENABLE_CHIP_SELECT_SC16IS7XX_0();
#endif
    SC16IS7XX_SPI_TDR = ucAddress;                                       // transmit the address
    while (!(SC16IS7XX_SPI_SR & SSI_RNE)) {                              // wait until tx byte has been sent and rx byte has been completely received
#ifdef _WINDOWS
        SC16IS7XX_SPI_SR |= SSI_RNE;
#endif
    };
    ulRxData = SC16IS7XX_SPI_RDR;                                        // reset receive data flag with dummy read - the rx data is not interesting here
    SC16IS7XX_SPI_TDR = 0xff;                                            // send dummy byte (idle line)
    while (!(SC16IS7XX_SPI_SR & SSI_RNE)) {};                            // wait until tx byte has been sent and rx byte has been completely received
    ulRxData = SC16IS7XX_SPI_RDR;                                        // read the returned data
#ifdef _WINDOWS
...
static void ext_sci_0_interrupt(void)
{
    ...
    while (!(CHECK_UART_0_1_INT_PENDING())) {                            // while the interrupt input stays low
        ...
        if ((fnCheckExtinterrupts(0) == 0) && (fnCheckExtinterrupts(1) == 0)) {
            fnSimulateInputChange(EXT_UART_0_1_INT_PORT, (7 - EXT_UART_0_1_INT_BIT), SET_INPUT); // remove interrupt if no more waiting
        }
    #endif
    }
}
...
{
    unsigned char ucInterrupt;
    while (!(CHECK_UART_2_3_INT_PENDING())) {                            // while the interrupt input stays low
...
//
extern void fnConfigExtSCI(QUEUE_HANDLE Channel, TTYTABLE *pars)
{
    ...
    interrupt_setup.int_port_sense = (IRQ_FALLING_EDGE);                 // interrupt on falling edge
    #if NUMBER_EXTERNAL_SERIAL > 2
    if (Channel & 0x02) {                                                // enable appropriate interrupt handler
        interrupt_setup.int_port = EXT_UART_2_3_INT_PORT;                // the port used
        interrupt_setup.int_port_bit = EXT_UART_2_3_INT_BIT;             // the input connected
        ...
        interrupt_setup.int_port_bit = EXT_UART_0_1_INT_BIT;             // the input connected
...
static void fnExtSCI_send(QUEUE_HANDLE Channel, unsigned char ucTxByte)
{
    ...
    #ifdef _WINDOWS                                                      // only for the simulator
    switch (Channel) {
    case 0:
