/**********************************************************************
   ...
   *********************************************************************
   ...
   07.10.2010 Add automatic power of 2 mode setting if the page size is defined for this {4}
   10.10.2010 Add managed memory support                                 {5}
    ...
    #endif
{
    ...
    switch (ucCommand) {
#if SPI_FLASH_PAGE_LENGTH == 256 || SPI_FLASH_PAGE_LENGTH == 512 || SPI_FLASH_PAGE_LENGTH == 1024 // {4}
    case POWER_OF_TWOS:
        ucTxCount = sizeof(ucCommandBuffer);
        break;
#endif
  ...
  //case ERASE_PROG_FROM_BUFFER_2:
    ...
    case BLOCK_ERASE:
        ...
        ucCommandBuffer[0] = (unsigned char)(ulPageNumberOffset >> 8);
        ucCommandBuffer[1] = (unsigned char)(ulPageNumberOffset);
        #else
        ucCommandBuffer[0] = (unsigned char)(ulPageNumberOffset >> 7);
        ucCommandBuffer[1] = (unsigned char)(ulPageNumberOffset << 1);
        #endif
...
#if SPI_FLASH_PAGE_LENGTH == 256 || SPI_FLASH_PAGE_LENGTH == 512 || SPI_FLASH_PAGE_LENGTH == 1024 // {4} the user wants to operate the device in power of 2 mode so check this and set if necessary
            fnSPI_command(READ_STATUS_REGISTER, 0, _EXTENDED_CS ucID, 1);// read the status register
            if (!(ucID[0] & 0x01)) {                                     // not configured for power of 2 page size
