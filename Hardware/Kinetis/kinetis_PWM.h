...
static __interrupt void _PWM_Interrupt_3(void)
{
    ...
    if (_PWM_TimerHandler[3] != 0) {                                     // if there is a user handler installed
...
#if defined _PWM_CONFIG_CODE
        {
            ...
            unsigned char ucFlexTimer = (ptrPWM_settings->pwm_reference >> 6);
            ...
            if (PWM_EXTERNAL_CLK == (ulMode & FTM_SC_CLKS_EXT)) {        // if external clock source is to be used program the clock input
                ...
                _CONFIG_PERIPHERAL(A, 12,  (PA_12_TPM_CLKIN0 | PORT_PS_UP_ENABLE)); // TPM_CKLIN0 on PA.12 (alt. function 2)
        #elif defined KINETIS_KL02 || defined KINETIS_KL04 || defined KINETIS_KL05
                _CONFIG_PERIPHERAL(A, 1,  (PA_1_TPM_CLKIN0 | PORT_PS_UP_ENABLE)); // TPM_CKLIN0 on PA.1 (alt. function 2)
        #else
                _CONFIG_PERIPHERAL(A, 18, (PA_18_FTM_CLKIN0 | PORT_PS_UP_ENABLE)); // FTM_CKLIN0 on PA.18 (alt. function 4)
        #endif
    #endif
            }
            ...
            if ((ulMode & PWM_DMA_CHANNEL_ENABLE) != 0) {
                ptrFlexTimer->FTM_channel[ucChannel].FTM_CSC |= FTM_CSC_DMA; // enable DMA trigger from this channel
            }
    #endif
