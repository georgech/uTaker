...
//
...
#elif defined MASK_1N71K                                                 // KL17Z128/KL17Z256/KL27Z128/KL27Z256/KL33Z128/KL33Z256/KL43Z128/KL43Z256/
    #define ERRATA_ID_8992                                               // AWIC: early NMI wakeup not detected upon entry to stop mode from VLPR mode
    #define ERRATA_ID_9380                                               // FlexIO: reading FlexIO register when FlexIO functional clock is disabled results in a bus hang
    #define ERRATA_ID_8777                                               // I2C: address match wake-up from low-power mode cannot receive data
    #define ERRATA_ID_9308                                               // I2C: I2C does not hold bus between byte transfers in receive and may result in lost data
    ...
    #define ERRATA_ID_8010                                               // LLWU: CMP flag in LLWU_Fx register cleared by multiple CMP out toggles when exiting LLSx or VLLSx modes
    #define ERRATA_ID_7950                                               // LLWU: when exiting from Low Leakage Stop (LLS) mode using the comparator, the comparator ISR is serviced before the LLWU ISR
    #define ERRATA_ID_10527                                              // LPUART: Setting and immediately clearing SBK bit can result in transmission of two break characters
    #define ERRATA_ID_7986                                               // LPUART: the LPUART_TX pin is tri-stated when the transmitter is disabled
    #define ERRATA_ID_7914                                               // PIT: after enabling the Periodic Interrupt Timer (PIT) clock gate, an attempt to immediately enable the PIT module may not be successful
    ...
    #define ERRATA_ID_8594                                               // ROM: I2C0_A1 register does not contain the reset default value when MCU boots from ROM
    #define ERRATA_ID_8595                                               // ROM: ROM read memory command may read incorrect data back from flash in some cases
    #define ERRATA_ID_10358                                              // SIM: a reset during system clock divider (SIM_CLKDIV1) update may cause the crossbar switch to stall
    #define ERRATA_ID_6396                                               // sLCD: LCD_GCR[RVTRIM] bits are in reverse order
    ...
    #define ERRATA_ID_6060                                               // TSI: out of range interrupt shows incorrect behavior with some configurations
#elif defined MASK_1N97F || defined MASK_2N97F
    ...
    #define ERRATA_ID_5472                                               // SMC: mode transition VLPR->VLLS0 (POR disabled)->RUN, will cause POR & LVD
    #define ERRATA_ID_5745                                               // SPI1: back to back DMA data transfers are not possible if the core:bus frequency ratio is greater than 
    #define ERRATA_ID_5490                                               // SPI1: DMA data transfers at the maximum baud rate can result in corrupted data
    #define ERRATA_ID_5515                                               // TSI: The end of scan flag is not automatically cleared when continuously scanning
    #define ERRATA_ID_5744                                               // UART0: receiver wakeup control bit cannot be set immediately after a wakeup event
    #define ERRATA_ID_5563                                               // UART0: when the Receiver Wake Up control bit is set, an IDLE condition is not detected correctly
    #define ERRATA_ID_5753                                               // UART1 / UART2: when the Receiver Wake Up control bit is set, an IDLE condition is not detected correctly
...
#elif defined MASK_0N65N                                                 // K65, K66, K26
    #define ERRATA_ID_8992                                               // AWIC: early NMI wakeup not detected upon entry to stop mode from VLPR mode
    #define ERRATA_ID_6939                                               // Core: interrupted loads to SP can cause erroneous behavior
    #define ERRATA_ID_6940                                               // Core: VDIV or VSQRT instructions might not complete correctly when very short ISRs are used
    #define ERRATA_ID_7993                                               // MCG: FLL frequency may be incorrect after changing the FLL reference clock
    ...
    #define ERRATA_ID_3983                                               // SDHC: problem when ADMA2 last descriptor is LINK or NOP
    #define ERRATA_ID_3978                                               // SDHC: software cannot clear DMA interrupt status bit after read operation
    #define ERRATA_ID_9861                                               // SMC: LLS3/LLS2 low power mode current draw at cold may be higher than specified on some parts
    #define ERRATA_ID_4647                                               // UART: flow control timing issue can result in loss of characters if FIFO is not enabled
    #define ERRATA_ID_9712                                               // USB-PHY: USB PHY PLL does not lock when MCGC2[EREFS] = 0 (external oscillator clock)
...
#elif defined MASK_1N87B                                                 // Kinetis KL17/KL27 32k and 64k flash
#elif defined MASK_1N87M
    #define ERRATA_ID_3863                                               // ADC: in 16-bit differential mode, ADC may result in a conversion error when positive input is near upper rail reference voltage
    #define ERRATA_ID_8992                                               // AWIC: early NMI wakeup not detected upon entry to stop mode from VLPR mode
    #define ERRATA_ID_8010                                               // LLWU: CMP flag in LLWU_Fx register cleared by multiple CMP out toggles when exiting LLSx or VLLSx modes
    #define ERRATA_ID_8594                                               // ROM: I2C0_A1 register does not contain the reset default value when MCU boots from ROM
    #define ERRATA_ID_8595                                               // ROM: ROM read memory command may read incorrect data back from flash in some cases
    #define ERRATA_ID_2580                                               // UART: start bit sampling not compliant with LIN 2.1 specification
#elif defined MASK_1N52N                                                 // Kinetis KL28Z512
    #define ERRATA_ID_9464                                               // FIRC: a transfer error is received when writing to the SCG_FIRCSTAT register
    #define ERRATA_ID_9380                                               // FlexIO: reading FlexIO register when FlexIO functional clock is disabled results in a bus hang
    #define ERRATA_ID_9364                                               // LPI2C: HS-mode signal on Repeated START uses Fast mode timing for the (Repeated) START hold time
    #define ERRATA_ID_9365                                               // LPI2C: master does not always end the transfer when the NACK detect flag is set
    #define ERRATA_ID_9881                                               // LPIT: LPIT does not stop in the Debug or Low Power modes regardless of the DBG_EN or DOZE_EN bits setting in the LPITx_MCR register when Timer Reload On Trigger is set
