/**********************************************************************
    ...
    *********************************************************************
    ...
    28.10.2011 Add STORAGE_REV_1 support                                 {1}
    17.12.2011 Return NO_SPI_FLASH_AVAILABLE when no answer to any ID request {2}
    ...
    #endif
{
    ...
    #ifdef _WINDOWS
    fnSimSST25(SST25_WRITE, (unsigned char)SPI_TDR);                     // simulate the SPI FLASH device
    SPI_SR |= SPI_RDRF;
    #endif
    ...
    #ifdef _WINDOWS
            SPI_RDR = fnSimSST25(SST25_READ, 0);                         // simulate the SPI FLASH device
...
#endif
{
    ...
    if ((ucID[0] == MANUFACTURER_ID_SST) && (ucID[1] == DEVICE_TYPE)) {  // SST memory part recognised
        ...
        if (ucID[0] == MANUFACTURER_ID_SST) {
            ...
            return NO_SPI_FLASH_AVAILABLE;                               // {2}
        }
    }
    ucID[0] = 0;
