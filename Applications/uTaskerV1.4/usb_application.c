...
#define EVENT_FFT_HALF_PING                 40
#define EVENT_FFT_PING                      41
#define EVENT_FFT_HALF_PONG                 42
#define EVENT_FFT_PONG                      43
...
#endif
#if defined USE_USB_AUDIO && defined AUDIO_FFT                           // {42}
...
extern void fnTaskUSB(TTASKTABLE *ptrTaskTable)
{
...
#endif
#if defined USB_HOST_SUPPORT || defined USE_USB_MSD || (defined USE_USB_CDC && !(defined FREE_RTOS_USB && USB_CDC_COUNT == 1))
    ...
    #endif
    #if defined USB_CDC_HOST
    if (USBPortID_comms[FIRST_CDC_INTERFACE] == NO_ID_ALLOCATED) {       // don't check bulk input queue until host is configured
        return;
    }
    #endif
...
static void fnInsertUSBKeyCode(CHAR cInputCharacter, unsigned char ucKeyboardState[8])
{
    ...
    else if ('?' == cInputCharacter) {        USB_queue = &(ptrUsbCDC_TxQueue->USB_queue);
        ucKeyboardState[0] = 0x02;                                       // shift
        cInputCharacter = 56;
    }
    else {
...
extern QUEUE_TRANSFER fnSendToUSB(unsigned char *ptrData, QUEUE_TRANSFER Length)
{
    #if defined USB_CDC_HOST
    int iResult;
    if (USBPortID_comms[FIRST_CDC_INTERFACE] == NO_ID_ALLOCATED) {
        #if defined USE_MAINTENANCE
        usUSB_state &= ~(ES_USB_RS232_MODE);
        #endif
    ...
    iResult = fnWrite(USBPortID_comms[FIRST_CDC_INTERFACE], ptrData, Length); // pass the buffer on to USB-CDC interface
    ...
    case (unsigned char)REMOTE_NDIS_QUERY_MSG:
        {
            ...
            if (ptrRNDIScontrol->ulMaxRNDISmessageLength >= ptrResponse->ucMessageLength[0]) { // if the host can accept the length
                ...
                ptrRNDIScontrol->encapsulatedResponseLength = (QUEUE_TRANSFER)ptrRNDIScontrol->ulMaxRNDISmessageLength;
if (ptrRNDIScontrol->encapsulatedResponseLength == 0) {
    fnDebugMsg("Zero!\r\n");
}
            }
...
static void fnSendEncapsulatedResponse(int iInterface, unsigned short usMaxDataLength)
{
    ...
    if (remoteNDIScontrol[iInterface].statusResponseLength != 0) {       // if a status change is to be reported
        ...
        fnWrite(USB_control, remoteNDIScontrol[iInterface].remoteNDISstatus, usMaxDataLength);
...
#if defined USE_USB_CDC && defined USB_CDC_RNDIS
                unsigned short usMaxDataLength = usExpectedData;
#endif
...
{
    #define _PLL_COMPENSATION  (PIT_FREERUN_FREQ(48000)/200)             // 0.5% adjustment
    ...
    if (slDelta > DRIFT_LIMIT_MAX) {
        PIT_LDVAL0 = (PIT_FREERUN_FREQ(48000) - PLL_COMPENSATION);       // speed up the PIT
    }
    else if (slDelta < DRIFT_LIMIT_MIN) {
        PIT_LDVAL0 = (PIT_FREERUN_FREQ(48000) + PLL_COMPENSATION);       // slow down the PIT
    }
    else {
        PIT_LDVAL0 = PIT_FREERUN_FREQ(48000);                            // set the exact frequency since the synchronisation is in the optimal region
    }
}
...
{
    PIT_LDVAL0 = PIT_FREERUN_FREQ(48000);                                // ensure that the PIT's center frequency is valid
