...
#define DMX512_DMX_RECEPTION_READY      3
...
/* =================================================================== */
    ...
    static int fnHandleDMX512_frame(QUEUE_HANDLE uart_handle, unsigned short usFrameLength);
    ...
    static void fnHandleRDM(QUEUE_HANDLE uart_handle, DMX512_RDM_PACKET *ptrPacket, unsigned short usPacketLength);
    static unsigned short fnDMX512_RDM_checksum(DMX512_RDM_PACKET *ptrPacket, unsigned short usPacketLength);
#endif
#if defined USE_DMX_RDM_MASTER
    static int fnSend_DMX512_RDM(QUEUE_HANDLE uart_handle, unsigned char ucCommandClass, unsigned short usParameterID, const unsigned char ucDestinationUID[6]);
...
static unsigned char ucDMX512_HW_event = 0;
...
#endif
#if defined USE_DMX_RDM_SLAVE
    static ucMuteFlag[DMX512_SLAVE_INTERFACE_COUNT] = {0};               // mute flag (reset by default and after a DISC_UN_MUTE message)
    ...
    static unsigned char ucUppedBoundUID[6] = {0};
...
extern void fnDMX512(TTASKTABLE *ptrTaskTable)
{
    ...
    if (ucDMX512_state == 0) {                                           // on initialisation
        ...
        tInterfaceParameters.Config |= (INFORM_ON_FRAME_TRANSMISSION);   // we require interrupt call back on the end of transmission (after second stop bit has completed)
    ...
    while (fnRead(PortIDInternal, ucInputMessage, HEADER_LENGTH) != 0) { // check task input queue
        ...
        case INTERRUPT_EVENT:
            ...
            case DMX512_TX_NEXT_TRANSMISSION:                            // DMX512 frame is starting so we should prepare the following frame's data content
                ...
                case 0:                                                  // DMX512 mode
                    usTxLength = fnConstructDMX512(ucDMX512_tx_buffer);
                    fnWrite(DMX512_Master_PortID, ucDMX512_tx_buffer, (DMX_SLOT_COUNT + 1 + 2)); // prepare next DMX512 frame
                    break;
                case DMX512_RDM_DISCOVERY:
                    fnSend_DMX512_RDM(DMX512_Master_PortID, DMX512_RDM_COMMAND_CLASS_DISCOVERY_COMMAND, DMX512_RDM_PARAMETER_ID_DISC_UNMUTE, cucBroadcast); // prepare a broadcast unmute so that all slaves will respond
                    uDisable_Interrupt();
                    ...
                    uMemset(ucLowerBoundUID, 0xff, 5);                   // initial upper bound
                    ucLowerBoundUID[5] = 0xfe;
                    break;
                case DMX512_RDM_DISCOVERING:
                    // Check whether slaves have been detected and choose the next bounds
                    //
                    if (fnHandleRDM_MasterRx(DMX512_Master_PortID, ucUID, 0) == 0) {
                        // Valid response
                        //
                        fnSend_DMX512_RDM(DMX512_Master_PortID, DMX512_RDM_COMMAND_CLASS_DISCOVERY_COMMAND, DMX512_RDM_PARAMETER_ID_DISC_MUTE, ucUID); // prepare a uniast mute to teh slave that we have discovered
                        uDisable_Interrupt();
                            ucDMX512_state &= ~(DMX512_RDM_STATE_MASK);
                ...
                case DMX512_RDM_GLOBAL_UNMUTE:
                    fnSend_DMX512_RDM(DMX512_Master_PortID, DMX512_RDM_COMMAND_CLASS_DISCOVERY_COMMAND, DMX512_RDM_PARAMETER_ID_DISC_UNIQUE_BRANCH, cucBroadcast); // prepare a discovery transmission
                    uDisable_Interrupt();
                        ucDMX512_state &= ~(DMX512_RDM_STATE_MASK);
            ...
            case DMX512_RDM_RECEPTION_READY:                             // an RDM message has been received which must be responed to
                fnHandleDMX512_frame(DMX512_Slave_PortID, usThisLength); // handle the received frame
                break;
    #endif
            default:
                break;
            ...
            case TTY_BREAK_FRAME_RECEPTION:
                {
                    ...
                    fnHandleDMX512_frame(DMX512_Slave_PortID, usFrameLength); // handle the received frame
                    break;
                }
            }
            break;
...
static void fnStartDelay(unsigned long ulDelay_us, void(*int_handler)(void))
{
    ...
    #if (defined USE_DMX_RDM_MASTER && defined UART_FRAME_COMPLETE)
...
{
    if ((ucDMX512_state & DMX512_RDM_STATE_MASK) > DMX512_RDM_DISCOVERY) { // if a RDM transmission was sent
...
static int fnHandleDMX512_frame(QUEUE_HANDLE uart_handle, unsigned short usFrameLength)
{
    ...
    case START_CODE_RDM:                                                 // (0xcc) RDM content
        fnHandleRDM(uart_handle, (DMX512_RDM_PACKET *)ptrRxFrame, usFrameLength);
        break;
    #endif
    default:
        break;
...
{
    if (iRxCount < sizeof(ucRxBuffer)) {
        ucRxBuffer[iRxCount] = data;
        if (iRxCount == 0) {                                           
            ucType = data;
        }
            ...
            case START_CODE_RDM:
                if (usThisLength == (unsigned short)iRxCount) {          // if a complete RDM packet has been received
                    if ((uMemcmp(ucSlaveSourceUID, &ucRxBuffer[3], 6) == 0) || (uMemcmp(cucBroadcast, &ucRxBuffer[3], 6) == 0)) { // if the slave is being addressed or a broadcast
                        ...
                        if (usCheckSum == fnDMX512_RDM_checksum((DMX512_RDM_PACKET *)ucRxBuffer, (usThisLength - 1))) { // check that the checksum is correct
                            usThisLength++;
                            fnStartDelay(PIT_US_DELAY(200), fnRDM_turnaround);   // 176us delay must be respected until the slave start sending either a packet of the break before it
...
{
    usThisLength = (unsigned short)iRxCount;
    iRxCount = 0;
}
    #endif
...
static int fnSend_DMX512_RDM(QUEUE_HANDLE uart_handle, unsigned char ucCommandClass, unsigned short usParameterID, const unsigned char ucDestinationUID[6])
{
    if (fnWrite(uart_handle, 0, (DMX_SLOT_COUNT + 1)) > 0) {             // if there is free buffer space
        ...
        unsigned char ucDMX512_RDM_buffer[DMX_SLOT_COUNT + 1];
        ...
        ptrRDMpacket->ucTransactionNumber = ucTransactionNumber;
        ...
        case DMX512_RDM_PARAMETER_ID_DISC_MUTE:                          // parameter IDs with no parameter data
        ...
        case DMX512_RDM_PARAMETER_ID_DISC_UNIQUE_BRANCH:
            ...
            uMemcpy((ptr_ucData + 6), ucUppedBoundUID, 6);
        ...
        *ptrCheckSum = (unsigned char)usCheckSum;
        ...
        return -1;                                                       // this must never happen in order for the state machine to operate correctly
    }
}
    ...
    while (fnRead(uart_handle, &ucRDM_response[iLength], 1) != 0) {      // extract the DMX512 reception frame one byte at a time
        if (iLength == 0) {
            if (ucUID != 0) {                                            // response to a discovery
                if (ucRDM_response[0] == 0xaa) {                         // pre-amble separator (this must be detected for the frame to be decoded)
                    unsigned short usCheckSum;
                ...
                else if (ucRDM_response[0] != 0xfe) {                    // pre-amble (there can be one or more pre-amble bytes)
                    break;
                }
            }
            else {
            ...
            if (iLength++ == 2) {                                        // if the message length is ready
                ...
                if (usReceivedChecksum == usChecksum) {
                    ...
                    if (usExpected == usPID) {
                        ...
                        iResult = 0;                                     // valid message content
                    }
                }
                break;
            }
