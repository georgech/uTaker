...
{
    uMemset( ctx, 0, sizeof( aes_context ) );
}
    ...
    unsigned char output[16])
{
    ...
    if (mode == AES_ENCRYPT) {
        ...
        mmcau_aes_decrypt(input, (const unsigned char *)(ctx->buf), (const int)(ctx->nr), output); // make use of crypto acceleration routines
    }
#else
    int i;
